  ...
  return (
    <div className="min-h-screen py-8">
      ...
      <div className="mt-8 p-6 bg-highlight-yellow text-primary-blue rounded-lg shadow-lg text-center">
        ...
        <button className="mt-4 bg-primary-blue text-white font-bold py-2 px-6 rounded-full hover:opacity-90 transition-opacity">
          Copy Code: BANKX50
        </button>
      </div>
      {/* Referral cards */}
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h3 className="text-xl font-semibold text-primary-blue mb-2">App/Bank Name {i + 1}</h3>
            ...
            <div className="flex items-center justify-between bg-gray-100 p-3 rounded-md mb-3">
              <span className="font-mono text-accent-green text-lg">REFCODE{i + 1}</span>
              <button className="text-primary-blue hover:text-accent-green transition-colors">Copy</button>
            </div>
            <a href="#" className="text-accent-green hover:underline font-medium">
